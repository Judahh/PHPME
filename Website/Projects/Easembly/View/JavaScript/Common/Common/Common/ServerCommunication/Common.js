...
function booleanNoCacheFile(file){
    if ((((file.indexOf('EditorSettings') !=-1)||(file.indexOf('CodeEditor') !=-1)) && editorLoaded==false)||(file.indexOf('FileTree') !=-1)){
        return true;
    }
    return false;
}
...
function request(element,file,format) {
    ...
    if (cached==null||cached==''||cached==undefined||booleanNoCacheFile(file)) {
        var ajaxRequest;
        try {
            // Opera 8.0+, Firefox, Safari
            ajaxRequest = new XMLHttpRequest();
